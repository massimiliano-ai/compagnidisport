 ...
 * Statistiche avanzate per organizzatori di attività
 */
...
class CDV_Organizer_Stats {
    ...
    public static function init() {
        ...
        add_action('wp_ajax_cdv_get_activity_analytics', array(__CLASS__, 'ajax_get_activity_analytics'));
    }
    ...
    public static function get_overview_stats($user_id) {
        ...
        $stats['total_travels'] = intval($wpdb->get_var($wpdb->prepare(
            ...
            WHERE post_type = 'attivita' AND post_author = %d AND post_status = 'publish'",
            $user_id
        )));
        ...
        $stats['active_travels'] = intval($wpdb->get_var($wpdb->prepare(
            ...
            LEFT JOIN {$wpdb->postmeta} pm ON p.ID = pm.post_id AND pm.meta_key = 'cdv_activity_status'
            WHERE p.post_type = 'attivita' AND p.post_author = %d AND p.post_status = 'publish'
        ...
        $stats['total_participants'] = intval($wpdb->get_var($wpdb->prepare(
            ...
            INNER JOIN {$wpdb->posts} p ON pt.activity_id = p.ID
        ...
        $stats['this_month_travels'] = intval($wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM {$wpdb->posts}
            WHERE post_type = 'attivita' AND post_author = %d
     ...
     */
    public static function get_activity_performance($user_id, $limit = 10) {
        global $wpdb;
        ...
        $activities = $wpdb->get_results($wpdb->prepare(
            "SELECT p.ID, p.post_title,
                ...
                (SELECT COUNT(*) FROM {$wpdb->prefix}cdv_participants
                 WHERE activity_id = p.ID AND status = 'accepted') as accepted_count,
                (SELECT COUNT(*) FROM {$wpdb->prefix}cdv_participants
                 WHERE activity_id = p.ID AND status = 'pending') as pending_count,
                p.post_date
            FROM {$wpdb->posts} p
            ...
            LEFT JOIN {$wpdb->postmeta} pm_status ON p.ID = pm_status.post_id AND pm_status.meta_key = 'cdv_activity_status'
            WHERE p.post_type = 'attivita' AND p.post_author = %d AND p.post_status = 'publish'
        ...
        foreach ($activities as &$activity) {
            $max = intval($activity['max_participants']) ?: 1;
            $accepted = intval($activity['accepted_count']);
            $activity['fill_rate'] = round(($accepted / $max) * 100, 1);
            $activity['potential_revenue'] = floatval($activity['budget']) * $accepted;
        }
    ...
    public static function get_monthly_stats($user_id, $months = 6) {
        ...
        for ($i = $months - 1; $i >= 0; $i--) {
            ...
            $activities = intval($wpdb->get_var($wpdb->prepare(
                "SELECT COUNT(*) FROM {$wpdb->posts}
                WHERE post_type = 'attivita' AND post_author = %d
        ...
        $destinations = $wpdb->get_results($wpdb->prepare(
            "SELECT pm.meta_value as destination, COUNT(*) as travel_count,
                    SUM((SELECT COUNT(*) FROM {$wpdb->prefix}cdv_participants
                         WHERE activity_id = p.ID AND status = 'accepted')) as total_participants
            FROM {$wpdb->posts} p
    ...
    public static function ajax_get_organizer_stats() {
        ...
        $travel_performance = self::get_activity_performance($user_id, 10);
     ...
     */
    public static function ajax_get_activity_analytics() {
        check_ajax_referer('cdv_ajax_nonce', 'nonce');
        ...
        $analytics['accepted'] = intval($wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM {$participants_table} WHERE activity_id = %d AND status = 'accepted'",
            $activity_id
        )));

        $analytics['pending'] = intval($wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM {$participants_table} WHERE activity_id = %d AND status = 'pending'",
            $activity_id
        )));

        $analytics['rejected'] = intval($wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM {$participants_table} WHERE activity_id = %d AND status = 'rejected'",
            $activity_id
        )));
        ...
        $analytics['wishlist_count'] = intval($wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM {$wishlist_table} WHERE activity_id = %d",
            $activity_id
        )));

        // Max participants and budget
        $analytics['max_participants'] = intval(get_post_meta($activity_id, 'cdv_max_participants', true));
        $analytics['budget'] = floatval(get_post_meta($activity_id, 'cdv_budget', true));
